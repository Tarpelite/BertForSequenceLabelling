...
from utils_sequence_labelling import (read_label_examples, convert_examples_to_features,
                                        RankPreProcessor, RankFeatures, RankExample, convert_examples_to_rank_features)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            # only caculate score on byte-level
            f = f1_score(eval_feature.label_seq, pred, average=None)
            if len(f) > 1:
                nb_f1_scores += f[1]
            r = recall_score(eval_feature.label_seq, pred, average=None)
            if len(r) > 1:
                nb_r_scores += r[1]
            p = precision_score(eval_feature.label_seq, pred, average=None)
            if len(p) > 1:
                nb_p_scores += p[1]
            ...
            doc_str = " ".join(words[:words_num])
            logits_str = " ".join([str(x) for x in words_logits[:words_num]])
            bin_str = " ".join([str(ws) for ws in words_scores[:words_num]])


            all_results.append([doc_str, logits_str, bin_str])
...
def rank_train(args, train_dataset, model, tokenizer):
    """ Train the model """
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter()
    ...
    # Train!
    logger.info("***** Running cls training *****")
    logger.info("  Num examples = %d", len(train_dataset))
    logger.info("  Num Epochs = %d", args.num_train_epochs)
    logger.info("  Instantaneous batch size per GPU = %d", args.per_gpu_train_batch_size)
    logger.info("  Total train batch size (w. parallel, distributed & accumulation) = %d",
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                        # Save model checkpoint
                        output_dir = os.path.join(args.output_dir, 'checkpoint-rank-{}'.format(global_step))
                        if not os.path.exists(output_dir):
                            os.makedirs(output_dir)
                        model_to_save = model.module if hasattr(model, 'module') else model  # Take care of distributed/parallel training
                        model_to_save.save_pretrained(output_dir)
                        torch.save(args, os.path.join(output_dir, 'training_args_rank.bin'))
                        logger.info("Saving model checkpoint to %s", output_dir)
...
def predict_and_rank(args, model, rank_model, tokenizer, prefix=""):
    dataset, examples, features = load_and_cache_examples(args, tokenizer, evaluate=True, output_examples=True)
    if not os.path.exists(args.output_dir) and args.local_rank in [-1, 0]:
        os.makedirs(args.output_dir)
...
def main():
    ...
    parser.add_argument('--checkpoint_rank', type=str, default='')
    ...
    ## load rank model
    rank_config_class, rank_model_class, rank_tokenizer_class = MODEL_CLASSES['bert-cls']
    rank_config = rank_config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path)
    rank_tokenizer = rank_tokenizer_class.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path)
    rank_model = rank_model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path), config=rank_config)
    ...
    if args.do_train:
        ...
        # Ranking Training
        rank_dataset = make_rank_train_set(args, model, tokenizer)
        global_rank_step, tr_rank_loss = rank_train(args, rank_dataset, rank_model, rank_tokenizer)
        logger.info(" global_rank_step = %s, average rank loss = %s", global_rank_step, tr_rank_loss)
    ...
    # Save the trained model and the tokenizer
        ...
        torch.save(rank_model.state_dict(), os.path.join(args.output_dir, "rank_model.bin"))
    ...
    if args.do_eval:
        if args.checkpoint:
            ...
            model.load_state_dict(state_dict)
            model.to(args.device)
        if args.checkpoint_rank:
            logger.info("loading rank model from %s", args.checkpoint_rank)
            rank_state_dict = torch.load(args.checkpoint_rank)
            rank_model.load_state_dict(rank_state_dict)
            rank_model.to(args.device)
            # Evaluate
        results = predict_and_rank(args, model, rank_model, tokenizer)
